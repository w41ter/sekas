...
/// Write partial of the iterator's data to the file, return `None` if all data
/// is written.
...
pub(crate) fn apply_snapshot(engine: &GroupEngine, replica_id: u64, snap_dir: &Path) -> Result<()> {
    if !snap_dir.is_dir() {
        ...
        return Err(Error::InvalidArgument(format!("{} is not a directory", snap_dir.display())));
    }
    ...
    for entry in snap_dir.read_dir()? {
        ...
        let path = entry.path();
        if is_sst_file(&path) {
            debug!("replica {replica_id} apply snapshot with sst file {}", path.display());
            files.push(path.file_name().unwrap().to_owned());
        }
    }

    if files.is_empty() {
        error!("replica {replica_id} apply snapshot {}: snap dir is empty", snap_dir.display());
        return Err(Error::InvalidArgument(format!("{} is empty", snap_dir.display())));
    }
...
mod tests {
    ...
    async fn create_engine(dir: &Path, group_id: u64, shard_id: u64) -> GroupEngine {
        ...
        let group_engine =
            GroupEngine::create(&EngineConfig::default(), db.clone(), group_id, 1).await.unwrap();
    ...
    fn put_data(engine: &GroupEngine, shard_id: u64, prefix: &str, num: usize) {
        ...
        for i in 0..num {
            ...
            engine.put(&mut wb, shard_id, key.as_bytes(), &value, 0).unwrap();
        }
