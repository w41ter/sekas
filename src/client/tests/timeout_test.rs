...
use sekas_client::error::retryable_rpc_err;
use sekas_client::{NodeClient, RequestBatchBuilder, RpcTimeout};
...
async fn rpc_timeout_report_canceled() {
    ...
    let client = NodeClient::new(channel);
    let req = RequestBatchBuilder::new(0).transfer_leader(1, 1, 1).build();
    match client.batch_group_requests(RpcTimeout::new(Some(Duration::from_millis(100)), req)).await
    {
        Ok(_) => unreachable!(),
        Err(status) => {
            assert!(
